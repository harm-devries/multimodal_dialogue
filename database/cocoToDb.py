...
def loadCategories(cur, data):
    ...
    for category in data["categories"]:
        ...
        supercategory[sc_name] = supercategory.get(sc_name, len(supercategory)+1)
    ...
    try:
        for name, index in supercategory.iteritems():
            cur.execute("""INSERT INTO object_supercategory (supercategory_id,name) VALUES (%s,%s)""", (index, name))
    ...
    for oneCategory in categories:
        oneCategory["supercategory_id"] = supercategory[oneCategory["supercategory"]]

    try:
        cur.executemany("""INSERT INTO object_category(category_id,supercategory_id,name) VALUES (%(id)s, %(supercategory_id)s, %(name)s)""", categories)
    except Exception as error:
...
def loadPictures(cur, data):
    ...
    try:
        ...
        for oneChunk in chunks(pictures, 500):
            ...
            args_str = ','.join(cur.mogrify("(%s,%s,%s,%s,%s,%s)", q) for q in queries)
            if args_str:
                cur.execute("INSERT INTO picture(picture_id, coco_url, flickr_url, file_name, height, width) VALUES " + args_str)
    ...
    try:
        for oneChunk in chunks(annotations, 500):
            ...
            args_str = ','.join(cur.mogrify("(%s,%s,%s,%s,%s,%s,%s)", q) for q in queries)
            cur.execute("INSERT INTO object (object_id, picture_id, category_id, segment, bbox, is_crowd, area) VALUES " + args_str)
...
try:
    ...
    conn = psycopg2.connect("dbname='testdb' user='fstrub' host='localhost' password='21914218820*I!'")
except:
