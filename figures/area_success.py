...
sns.distplot(np.log(sum_incomplete)  , bins=rng, kde=False, label="Incomplete", color="g", hist_kws={'alpha':0.8})
sns.distplot(np.log(sum_failure)     , bins=rng, kde=False, label="Failure"   , color="r", hist_kws={'alpha':0.8})
f =  sns.distplot(np.log(sum_success), bins=rng, kde=False, label="Success"   , color="b", hist_kws={'alpha':0.8})
...
f = sns.regplot(x=np.linspace(4, 13, 8), y=curve, order=3, scatter=False, line_kws={'linestyle':'--'},ci=None, truncate=False, color="grey")
ax2.set_xlim(4,12)
ax2.set_ylim(0,1)
ax2.grid(None)
